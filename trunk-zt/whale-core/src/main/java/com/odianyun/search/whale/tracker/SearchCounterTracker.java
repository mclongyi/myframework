...
public class SearchCounterTracker {
   ...
   // private Set<String> noResultSet = new HashSet<>();
        ...
        timer.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                if (noResultSearchCount.intValue() != 0 && noResultSearchCount.floatValue() / totalSearchCount.intValue() > noResultThreshold) {
                    ...
                    emailUtil.sendMail("关键词查询出错", noResultMap);
                }
    ...
    public synchronized void checkHitNum(long hitNum,BaseSearchRequest searchRequest){
        totalSearchCount.getAndIncrement();
        if(hitNum == 0){
            noResultSearchCount.getAndIncrement();
            if(noResultSearchMap.containsKey(searchRequest.toString())){
                Integer count = noResultSearchMap.get(searchRequest.toString());
                count ++;
                noResultSearchMap.put(searchRequest.toString(),count);
            }else{
                noResultSearchMap.put(searchRequest.toString(),1);
            }

            //noResultSet.add(searchRequest.toString());
        }
    }
