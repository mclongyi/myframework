...
public class CategoryResponseHandler implements ResponseHandler {
	...
	public void handle(org.elasticsearch.action.search.SearchResponse esSearchResponse,
		...
		InternalTerms longTerms=(InternalTerms) aggregationMap.get(IndexFieldConstants.CATEGORYID);

		if (longTerms != null) {
			/*生成TreeResult的树状结构*/
			Map<Long, CategoryTreeResult> cateTreeResultMap = new HashMap<Long, CategoryTreeResult>();
			List<Terms.Bucket> counts=longTerms.getBuckets();
	...
	public void calcCategoryTree(long categoryId, long count,
		...
		List<Category> categoryList = categoryService
				.getFullPathCategory(categoryId,companyId);
